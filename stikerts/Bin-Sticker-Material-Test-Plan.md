...
*Prepared July 2026 · Goal: pick a durable, cost-effective vinyl + laminate source before committing to bulk ordering, without waiting the full advertised durability window.*

**Size note (update):** the product has since moved to 100×140mm, pack of 4, printed 4-up on a single A4 sheet — this supersedes the 30×20cm single-large-sticker assumption implied elsewhere. It doesn't change Tests 1–5 (same materials, same finish), but it does change what a "sample" looks like for Test 6 below (a 4-up sheet rather than one large sticker) and it's the reason Test 6 was added.
...
|---|---|---|
| Product | Printable Waterproof Sticker Set – A4 (6 ct), **White** | Glossy White Vinyl A4 + Matte Self-Adhesive Laminate A4 |
...
| Cost per page | £1.87 | £0.55–£1.14 depending on bulk tier |

**Note:** the Cricut 20-ct set (£0.65/page) is cheaper but transparent-only in the UK — not usable for an opaque bin sticker design, so it's excluded from this comparison. Both materials being tested here are white/opaque, making this a fair like-for-like test for the actual finished product.
...
- [ ] Laminate using your actual laminator (or by hand, if the laminator hasn't arrived yet — note which method was used, since it affects bubble/wrinkle risk independently of the material)
...
### Test 6 — Transit/shipping stress (NEW — added for the 100×140mm size change)
...
**Packaging comparison matrix (four candidates):**

| Packaging | Cost/pack | Packed thickness | Royal Mail format | 2nd Class postage | Protection | Presentation |
|---|---|---|---|---|---|---|
| Kraft envelope + tissue (flat wrap) | ~£0.08 | <1mm | **Letter** | ~85–91p | Lowest — sheet can flex/crease in a stuffed mailbag | Nice, simple |
| Kraft envelope + rigid backing card (no padding) | ~£0.09 | ~0.5mm | **Letter** | ~85–91p | Bend-resistant (card stops flexing), but no cushioning against knocks/crushing | Simple, slightly sturdier feel |
...
| C6 box (163×112×20mm, rigid) | ~£0.14 | 20mm | Large Letter | £1.55 | Best — resists bending and crushing entirely | Strongest "unboxing" moment |
...
**Set-up:** for each material (Cricut / Stickiply) prepare 4 more samples at the actual production size/layout (a 4-up sheet, not a single cut sticker), packed exactly as a real order would be — **four packaging candidates**, not three:
...
- [ ] **Post-to-self test:** address and post all four packed samples to yourself (or a friend/family member) via standard 2nd class. This is the single most realistic test available — it goes through actual sorting machines, bags, and delivery handling.
- [ ] **Measure packed thickness** of each with a ruler before posting — confirm which candidates actually stay under 5mm (Letter) vs which are inherently Large Letter, since this affects the postage cost comparison as much as the protection result
...
**If Cricut visibly outperforms:** the price premium may be justified for a product where durability is the core sales promise (per your competitor research — The Range's 10-year guarantee is the bar).
...
## Addendum (July 2026) — Cutting tool verification: corner punch & peel-nick
...
**Why this needs testing:** the corner punch and peel-nick technique are both proven on plain paper/cardstock, but your finished bin sticker is a laminated vinyl stack (vinyl + adhesive + self-adhesive laminate), which is thicker and tougher than what either tool is normally used on. Neither has been verified on your actual material yet.
