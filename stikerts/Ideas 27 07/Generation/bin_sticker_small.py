"""
Bin sticker generator — SMALL (140x100mm) — Kerbside Craft Co.

This is the original 25-style landscape catalogue, split out Sep 2026
into its own script so it can't be broken by work on a different size
(e.g. Medium, 210x140mm, in bin_sticker_medium.py). Shared plumbing --
icon caching, accent palettes, base/border drawing, vector icon shapes,
font-fit/curved-text helpers, and the generic grid sheet layout -- lives
in bin_sticker_core.py; this file only has what's genuinely specific to
this size: every style's own pixel-measured constants and drawing logic,
plus this size's STYLES/STYLE_LABELS/STYLE_PRODUCT_ID registries.

Every style here is landscape, 140x100mm, 4-per-A4 in a 2x2 grid,
printed on white vinyl. Same cut-guide-lines-and-tick-marks convention
as thankyou_card.py (guillotine + corner rounder punch + Slice 00200
workflow -- this PDF does NOT use Cricut Print Then Cut / registration
marks).

The original 10 PORTRAIT presets (100x140mm -- classic, minimal,
floral, recycle, house, reverse_block, split_panel, vintage,
corner_flourish, paw) were removed Sep 2026, before this file split --
this generator (and its predecessor single-file version) has been
landscape-only ever since. None of the 10 had shipped as a catalogued
product. If a portrait line is ever wanted again, restore from git
history / a prior chat export rather than rewriting from scratch --
those styles were print-tested and iterated on.
...
Every sticker is bordered (single or double line, or a filled colour
block which reads as its own border) -- see chat history for why "no
border" was dropped as a differentiator (p09a_borderless is the one
deliberate exception, kept as a DRAFT).
"""
...
# Portrait 100x140mm size -- no style in this file uses this. Kept as a
# sentinel so tests can assert PORTRAIT_STYLES is empty
# (STYLE_CARD_SIZE[s] == (CARD_W, CARD_H)) and so a revived portrait line
# has the original dimensions in one place.
CARD_W, CARD_H = 100 * mm, 140 * mm

# This size's actual card. Named P02_* for history -- P02 (house_banner)
# was the first landscape card in the original single-file design.
P02_CARD_W = 140 * mm
P02_CARD_H = 100 * mm

# ---------------------------------------------------------------------------
# P02 (house_banner) icon caching -- style-specific (not core material,
# since P02_ICON_MASTER is this one style's own source art), but defined
# up here because it's referenced from inside _style_p02_house_banner
# further down.
# ---------------------------------------------------------------------------
...
P25_BORDER_RADIUS = core.BORDER_CORNER_RADIUS  # same 7mm punch radius as draw_border
...
# an entry here. If you build another finished design, add it here too,
# or STYLE_LABELS and the products gallery will drift apart.
STYLE_PRODUCT_ID = {
    "house_banner": "D01",
...
# Every style in this file is this one size -- no per-style lookup dict
# needed the way the old multi-size single-file design required (that
# dict existed only to distinguish portrait from landscape styles within
# one shared registry; splitting by file makes that distinction
# structural instead of a runtime lookup). Kept as a dict (not a bare
# tuple) anyway so any code ported from the old single-file version that
# does STYLE_CARD_SIZE.get(style, ...) keeps working unchanged.
...
# This size's real print layout: 4-up, 2x2 grid, on a landscape A4 page.
# 14mm asymmetric LEFT margin (not centred) -- set from a real print test
# (Aug 2026): more slack on the left gives room for lamination
# misalignment without touching the stickers; less on the right since
# that side doesn't need the buffer. Works because this size has exactly
# 17mm of total horizontal slack on a 297mm-wide landscape page
...
def render_sheet(orders, out_path, caption=False):
    """Fills one landscape A4 sheet, 2x2 (up to 4 cards). Set
    caption=True to print each design's label in the bottom margin (used
    for the design gallery, not for real customer orders)."""
    core.render_sheet_grid(
        orders, out_path, P02_CARD_W, P02_CARD_H, cols=2, rows=2,
        draw_fn=draw_sticker, page_size=_PAGE_SIZE,
        margin_x=LANDSCAPE_MARGIN_LEFT, caption=caption,
...
def render_gallery(style_keys, sample_order, out_path):
    ...
    of them, paginated 4-up across as many landscape A4 pages as
    needed."""
    core.render_gallery_grid(
        style_keys, sample_order, out_path, P02_CARD_W, P02_CARD_H,
        cols=2, rows=2, draw_fn=draw_sticker, page_size=_PAGE_SIZE,
        style_labels=STYLE_LABELS,
...
# rationale. These are each style's REAL accent-border values, matching
# exactly what that style's own draw_border call actually uses -- not a
# generic guess. None means no real border to show (p09a_borderless has
# none by design; p25b_landscape_flourish's border is bespoke corner-
# bracket artwork, structurally too different from a simple rounded
# rectangle to represent here).
STYLE_PAD_RADIUS = {
...
if __name__ == "__main__":
    ...
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bin_sticker_small_gallery.pdf")
